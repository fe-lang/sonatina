...
use super::{LateCleanupProfile, opcode::OpCode};
...
impl LateBlockMerge {
    ...
    fn run_exact_block_dedup(
        ...
        loop {
            ...
            let mut groups: FxHashMap<Vec<InstKey>, Vec<BlockId>> = FxHashMap::default();
            ...
            for &block in block_order.iter() {
                ...
                groups.entry(region.key).or_default().push(block);
            }

            let mut aliases = FxHashMap::default();
            for blocks in groups.into_values() {
                if blocks.len() < 2 {
                    continue;
                }
