...
use sonatina_ir::{
    ir_writer::{DebugProvider, FuncWriteCtx, InstStatement, WriteWithFunc},
    module::FuncRef,
    BlockId, Function, U256,
};
    ...
    Op: std::fmt::Debug,
{
    pub fn print(
        ...
        for (funcref, layout) in self.functions.iter() {
            // xxx debug provider
            let ctx = FuncWriteCtx::with_debug_provider(&functions[*funcref], *funcref, dbg);
            layout.write(&ctx, w)?;
        }
        Ok(())
    }
...
impl<Op> WriteWithFunc for FuncLayout<Op>
where
    Op: std::fmt::Debug,
{
    fn write(
        &self,
        ctx: &sonatina_ir::ir_writer::FuncWriteCtx,
        w: &mut impl std::io::Write,
    ) -> std::io::Result<()> {
        // Mostly copied from VCode::print

        write!(w, "// ")?;
        ctx.write_function_signature(w)?;
        writeln!(w)?;
        writeln!(w, "{}:", ctx.func.sig.name())?;
