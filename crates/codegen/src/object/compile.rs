...
use super::{
    CompileOptions, EvmLoweringBackend, LoweredFunction, SectionLoweringCtx,
    artifact::ObjectArtifact,
    error::ObjectCompileError,
    link::{Atom, link_section},
    resolve::{ObjectId, ResolvedProgram, SectionId},
};

fn compile_required_sections_into_cache<B: EvmLoweringBackend>(
...
pub fn compile_all_objects<B: EvmLoweringBackend>(
    module: &Module,
    backend: &B,
    opts: &CompileOptions,
...
pub fn compile_object<B: EvmLoweringBackend>(
    module: &Module,
    backend: &B,
    object: &str,
    opts: &CompileOptions,
...
fn compile_section<B: EvmLoweringBackend>(
    program: &ResolvedProgram<'_>,
    backend: &B,
    ...
    let section_ctx = SectionLoweringCtx {
        ...
        push_width_policy: opts.push_width_policy,
    };
    ...
    if let Some(artifact) = backend
        .compile_section(program.module, &funcs, &data, &embeds, &section_ctx, opts)
        .map_err(|e| {
            vec![ObjectCompileError::LinkError {
                object: object_name.clone(),
                section: section_name.clone(),
    ...
    for func in funcs {
        let lowered: LoweredFunction = backend
            .lower_function(program.module, func, &section_ctx)
            .map_err(|e| {
        ...
        atoms.push(Atom::Code {
            func,
            bytes: lowered.bytes,
            fixups: lowered.fixups,
        });
    }

    for (gv, bytes) in data {
        atoms.push(Atom::Data { gv, bytes });
    }

    for (symbol, bytes) in embeds {
        atoms.push(Atom::Embed { symbol, bytes });
    }

    let artifact =
        link_section(&atoms, opts.push_width_policy, opts.emit_symtab).map_err(|msg| {
            vec![ObjectCompileError::LinkError {
                object: object_name.clone(),
                section: section_name.clone(),
                message: msg,
...
mod tests {
    use super::*;
    use crate::object::{
        CompileOptions, EvmLoweringBackend, Fixup, FixupFormat, FixupKind, LoweredFunction,
        PushWidthPolicy, SymbolId,
    };
    use sonatina_ir::{
        InstDowncast, Module,
        inst::data::{SymAddr, SymSize, SymbolRef},
        module::FuncRef,
    };
    ...
    impl EvmLoweringBackend for FakeBackend {
        type Error = String;
        ...
        fn lower_function(
            ...
            let width_hint: u8 = match section_ctx.push_width_policy {
                PushWidthPolicy::Push4 => 4,
                PushWidthPolicy::MinimalRelax => {
                    return Err("PushWidthPolicy::MinimalRelax is not implemented".to_string());
                }
            };
            let placeholder_len = 1usize + width_hint as usize;

            module.func_store.view(func, |func| {
                let is = func.inst_set();
                for block in func.layout.iter_block() {
                    for inst_id in func.layout.iter_inst(block) {
                        let inst = func.dfg.inst(inst_id);
                        ...
                        if let Some(sym) = <&SymAddr as InstDowncast>::downcast(is, inst) {
                            ...
                            bytes.extend(std::iter::repeat_n(0u8, placeholder_len));
                            fixups.push(Fixup {
                                kind: FixupKind::SymAddr(symbol_id(sym.sym())),
                                at,
                                width_hint,
                        ...
                        if let Some(sym) = <&SymSize as InstDowncast>::downcast(is, inst) {
                            ...
                            bytes.extend(std::iter::repeat_n(0u8, placeholder_len));
                            fixups.push(Fixup {
                                kind: FixupKind::SymSize(symbol_id(sym.sym())),
                                at,
                                width_hint,
                                format: FixupFormat::EvmPush,
                            });
            ...
            Ok(LoweredFunction { bytes, fixups })
        }
    }
...
"#;
        ...
        impl EvmLoweringBackend for RecordingBackend {
            type Error = String;
