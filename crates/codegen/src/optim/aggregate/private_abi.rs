...
pub(crate) trait PrivateAbiPlan {
...
pub(crate) fn retain_higher_order_safe_plans<P: PrivateAbiPlan>(
    module: &Module,
...
pub(crate) fn propagate_private_abi_types(
    module: &Module,
    ...
    let mut types = PrivateAbiTypeRewriter::new(module, old_sigs);
...
fn rewrite_global_types(module: &Module, types: &mut PrivateAbiTypeRewriter) {
...
fn rewrite_function_types(
    ...
    struct TypeVisitor<'a> {
        ctx: ModuleCtx,
        types: &'a mut PrivateAbiTypeRewriter,
    }
...
fn collect_rewrite_class_info<P: PrivateAbiPlan>(
    ctx: &ModuleCtx,
    plans: &FxHashMap<FuncRef, P>,
    ...
    for (&old_ty, class) in &mut info {
        ...
        for &func in &class.funcs {
            ...
            if let Some((ref_args, ref_rets, _)) = &reference {
                if ref_args.as_slice() != plan.new_arg_tys()
                    || ref_rets.as_slice() != plan.new_ret_tys()
                {
                    class.consistent_new_shape = false;
                    break;
                }
                ...
                reference = Some((
                    plan.new_arg_tys().to_vec(),
                    plan.new_ret_tys().to_vec(),
                    func,
                ));
            }
        }
...
fn collect_planned_func_types<P: PrivateAbiPlan>(
    ctx: &ModuleCtx,
    plans: &FxHashMap<FuncRef, P>,
