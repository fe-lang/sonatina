...
impl LocalStack {
    ...
    pub fn freeze(&mut self, mut values: BitSet<Value>) {
        if values.is_empty() {
            self.unfreeze();
            return;
        }
    ...
    fn top_n(&self, n: usize) -> impl Iterator<Item = &'_ Value> + '_ {
    ...
    pub fn ret(&mut self, act: &mut Actions, memory: &[MemSlot], return_val: Option<Value>) {
    ...
    pub fn retain(&mut self, act: &mut Actions, args: &[Value], live_out: &BitSet<Value>) {
        ...
        F: Fn(Value) -> bool,
    {
        ...
        let Some(pos) = self.position_within_reach(|v| {
            poppable(v)
                && (self.dead.contains(v) || dfg.is_imm(v) || memory_slot_of(v, memory).is_some())
        }) else {
            return false;
        };
...
mod tests {
    ...
    fn clean_top_of_stack() {
        let mut stack =
            LocalStack::with_values(&[Value(1), Value(2), Value(3), Value(4), Value(5)]);
        let live = BitSet::from([Value(2), Value(5)]);

        let mut act = Actions::default();
        stack.clean_top_of_stack(&mut act, |val, _| !live.contains(val));

        assert_eq!(stack.stack, VecDeque::from([Value(2), Value(5)]));
    }
}
