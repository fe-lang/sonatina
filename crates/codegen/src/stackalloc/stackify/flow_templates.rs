...
use super::{
    ...
    builder::{StackifyContext, StackifyOperandMode},
    iteration::{
        ...
        operand_order_for_evm, operand_order_for_evm_machine, skip_pre_exit_cleanup,
    },
...
pub(super) fn solve_templates_from_flow(
    ...
    terminal_tail_insts: &SecondaryMap<InstId, bool>,
...
impl<'a, 'ctx> FlowTemplateSolver<'a, 'ctx> {
    ...
    fn simulate_block_and_record_edges(&mut self, block: BlockId, template: &BlockTemplate) {
        ...
        let terminal_tail_insts = self.terminal_tail_insts;
        ...
        for inst in ctx.func.layout.iter_inst(block) {
            ...
            if is_normal {
                args = match ctx.operand_mode {
                    StackifyOperandMode::HighEvm => {
                        operand_order_for_evm(ctx.func, inst, &ctx.value_aliases)
                    }
                    StackifyOperandMode::EvmMachine => {
                        operand_order_for_evm_machine(ctx.func, inst, &ctx.value_aliases)
                    }
                };
                consume_last_use =
            ...
            with_planner(ctx, mem, &mut stack, &mut actions, |planner| {
                let operand_prep_policy = if terminal_tail_insts[inst] && !is_call {
                    OperandPrepPolicy::TerminalDeadTail
                } else {
                    OperandPrepPolicy::Normal
                };
                planner.prepare_operands_for_inst(inst, &mut args, last_use, operand_prep_policy)
            });
