...
use sonatina_codegen::{
    ...
    object::{CompileOptions, EvmObjectBackend, PushWidthPolicy, compile_object},
...
fn test_evm(fixture: Fixture<&str>) {
    ...
    let obj_backend =
        EvmObjectBackend::new(EvmBackend::new(Evm::new(sonatina_triple::TargetTriple {
            architecture: Architecture::Evm,
            vendor: Vendor::Ethereum,
            operating_system: OperatingSystem::Evm(sonatina_triple::EvmVersion::Cancun),
        })));
    ...
    let opts = CompileOptions {
        push_width_policy: PushWidthPolicy::MinimalRelax,
        emit_symtab: false,
    };

    let artifact = compile_object(&parsed.module, &obj_backend, "Contract", &opts).unwrap();
